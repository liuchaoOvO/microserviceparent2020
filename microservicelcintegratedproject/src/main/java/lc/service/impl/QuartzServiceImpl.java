...
import lc.job.QuartzMainJobFactory;
import lc.service.quartz.*;
...
public class QuartzServiceImpl implements QuartzService, InitializingBean {
    ...
    @Override
    @Transactional
    public String startJob(String taskNo) throws Exception {
     ...
     */
    @Override
    public void initLoadOnlineTasks() throws Exception {
    ...
    public String runTaskRightNow(String taskNo) {
        ...
        try {
            ...
            if (ResultEnum.HTTP.getMessage().equals(sendType)) {
                try {
                    HttpClientUtil.doPost(url, "application/json;charset=UTF-8", executeParameter);
                    logger.info("");
                } catch (Exception ex) {
                    logger.error("");
